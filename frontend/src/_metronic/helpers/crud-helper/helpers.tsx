...
function stringifyRequestQuery(state: QueryState): string {
  console.log('🔍 stringifyRequestQuery called with state:', JSON.stringify(state, null, 2))
  ...
  // ✅ ใช้ key=value โดยตรง
  const filterEntries = state.filter 
    ? Object.entries(state.filter)
        .filter(([_, value]) => isNotEmpty(value))
        .map(([key, value]) => {
          const encodedValue = encodeURIComponent(String(value))
          console.log(`  → Adding filter: ${key}=${encodedValue} (original: ${value})`)
          return `${key}=${encodedValue}`
        })
    : []

  const allParts = [...filterEntries, pagination, sort, search]
  const result = allParts.filter(f => f).join('&')
