...
export interface Event {
  ...
  user_id: string | User
  event_name: string
  event_type_id: ID | { _id: ID; event_type_name: string; event_type_color?: string }
  person_in_charge: ID[] | User[]
  description?: string
  start_date: Date | string
  end_date: Date | string
  google_calendar_event_id?: string // ✅ เพิ่ม field นี้
  status: 'pending' | 'approved' | 'rejected' | 'cancelled' | string
  approvedBy?: ID | User | null
  comment?: string
  createdAt?: Date | string
  updatedAt?: Date | string
  // ✅ เพิ่มสำหรับ participation status
  require_participation_response?: boolean
  person_in_charge_details?: Array<{
    user_id: User | ID
    participation_status: 'not_required' | 'pending' | 'accepted' | 'declined'
