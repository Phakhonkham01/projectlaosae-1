...
export const checkGoogleCalendarStatus = async (userId: string): Promise<any> => {
...
export const getGoogleCalendarAuthUrl = async (userId: string): Promise<string> => {
  console.log('🔗 Getting Google Calendar auth URL for user:', userId)
...
export const syncEventToGoogleCalendar = async (
  ...
  console.log('🔄 Syncing event to Google Calendar:', { eventId, userId })
  
  try {
    const response = await axiosInstance.post(`/google/sync-event/${eventId}`, {
      user_id: userId
    })
    
    console.log('✅ Event synced to Google Calendar:', response.data)
    return response.data
  } catch (error: any) {
    console.error('❌ Sync to Google Calendar error:', {
      eventId,
      userId,
      response: error.response?.data,
      status: error.response?.status
...
export const unsyncEventFromGoogleCalendar = async (
  eventId: string,
  userId: string
): Promise<any> => {
  console.log('🗑️ Removing event from Google Calendar:', { eventId, userId })
  
  try {
    const response = await axiosInstance.delete(`/google/unsync-event/${eventId}`, {
      params: { user_id: userId }
    })
    
    console.log('✅ Event removed from Google Calendar:', response.data)
    return response.data
  } catch (error: any) {
    console.error('❌ Remove from Google Calendar error:', {
      eventId,
      userId,
      response: error.response?.data,
      status: error.response?.status
...
export const disconnectGoogleCalendar = async (userId: string): Promise<any> => {
  ...
  try {
    ...
    console.log('✅ Google Calendar disconnected:', response.data)
    return response.data
  } catch (error) {
...
// ✅ เพิ่มใน _requests.ts ของ Holiday module

/* =========================
   HOLIDAY GOOGLE CALENDAR REQUESTS
========================= */

// Sync Holiday to Google Calendar
export const syncHolidayToGoogleCalendar = async (
  holidayId: string, 
  userId: string
): Promise<any> => {
  console.log('🔄 Syncing holiday to Google Calendar:', { holidayId, userId })
  
  try {
    const response = await axiosInstance.post(`/google/sync-holiday/${holidayId}`, {
      user_id: userId
    })
    
    console.log('✅ Holiday synced to Google Calendar:', response.data)
    return response.data
  } catch (error: any) {
    console.error('❌ Sync holiday to Google Calendar error:', {
      holidayId,
      userId,
      response: error.response?.data,
      status: error.response?.status
...
export const unsyncHolidayFromGoogleCalendar = async (
  holidayId: string,
  userId: string
): Promise<any> => {
  console.log('🗑️ Removing holiday from Google Calendar:', { holidayId, userId })
  
  try {
    const response = await axiosInstance.delete(`/google/unsync-holiday/${holidayId}`, {
      params: { user_id: userId }
    })
    
    console.log('✅ Holiday removed from Google Calendar:', response.data)
    return response.data
  } catch (error: any) {
    console.error('❌ Remove holiday from Google Calendar error:', {
      holidayId,
      userId,
      response: error.response?.data,
      status: error.response?.status
